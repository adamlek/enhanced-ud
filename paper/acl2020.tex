...
Shared Task on parsing enhanced universal dependencies. We
develop a tree-to-graph transformation algorithm based on
dependency patterns. We show that with a simple method we are
able to achieve strong performance for enhanced edges, and
varying performance for the label enhancements depending on the
target language. For the official test data our ELAS score is
$67.85$ and our EULAS score is $80.18$. We analyse our system and
find key problems related to case marking that are responsible
for some of the label errors.  Additionally, we show that much of
the information needed to construct EUD graphs from UD trees are
present in the UD trees, and identify which phenomena require
...
More precisely, running our system on representative gold parses
will indicate the upper bound for our method. But in terms of
information contained in UD relevant to EUD, this experiment
indicate the lower bound of information contained in UD (for
these representative inputs).
...
Our main venue for future work is combining our simple, yet
effective method, with deep learning for the problematic cases
that are out of reach. These cases include ellipsis, multi-word
tokens, objects of conjoined verbs.
