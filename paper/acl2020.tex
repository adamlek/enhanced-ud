...
and so on.  A consequence of the additional edges added to the
basic UD tree is that EUD structures are graphs rather than
trees. Because of this, transition parsers cant generade EUD
graphs. Graph based parsers still work, but need to tweaking to
allow for multiple incomming edges.
...
universal dependencies to derive enhanced dependency graphs.

We chose this approach because in many cases enhanced dependencies
are functions of the basic dependencies. This means that most of
the enhancements to UD trees are already contained in the basic
trees. With this in mind a rule-based approach is attractive, our
approach is very efficient and works fine on just a CPU.

The hypothesis we ask in this paper is the following:    
\begin{enumerate}
    \item Most information about EUD is contained in UD
\end{enumerate}
We are primarily interested in how much of the enhancements are
already contained in UD and can thus be easily extracted usig a
simple and efficient approach.

We explore this by using a pre-trained dependency parser and
rewriting the trees, aswell as applying our method to gold UD
trees.
...
To obtain basic UD tree we use The Stanford Biaffine Dependency
parser. It is a graph-based dependency parser with biaffine
attention. ....
...
\begin{table}[ht]
	...
	\end{tabular}
	\caption{Coarse ELAS and EULAS on the gold trees in the development and test data.}
\end{table} 
%TODO: Add std dev.
